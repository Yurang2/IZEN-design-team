...
## 최우선 작업 승인 규칙
- 이 저장소에서는 사용자의 명시적 승인 전에는 파일 수정, 구현, 설정 변경, 스크립트 실행, Notion DB 변경을 하지 않는다.
- 버그 제보, 증상 설명, 질문, 원인 분석 요청은 자동으로 수정 승인으로 간주하지 않는다.
- 작업 전에는 먼저 원인/범위/변경 계획을 짧게 보고하고, 사용자가 "승인", "진행", "수정해", "반영해"처럼 명시적으로 허가한 뒤에만 실제 작업을 시작한다.
- 작업 중 새 범위나 추가 개선안이 발견되면 즉시 멈추고 변경 계획을 다시 보고한 뒤 재승인을 받는다.
- 읽기 전용 조회와 상태 확인은 가능하지만, 그 결과 실제 변경이 필요하면 반드시 승인 절차로 전환한다.
- 승인된 변경 작업이 완료되면 커밋과 푸시는 별도 재승인 없이 자동으로 진행하고, 결과는 사후보고한다.
...
- 사용자의 명시적 승인을 받기 전에는 파일 수정, 구현, 설정 변경, 스크립트 실행, Notion DB 변경 등 실제 작업을 시작하지 않는다.
...
2. **앱 상태/대량·append 위주 데이터**(스토리보드 본문, 로그, 업로드 기록, 이벤트)는 D1에 둔다.
