# Reference Notion DB / Storyboard D1 Storage

## Purpose
- 레퍼런스 자료함은 이미지, YouTube, 일반 링크, 기타 자료를 Notion에 저장한다.
- 스토리보드 문서는 PPTX export 전 웹 편집 상태를 D1/R2에 저장한다.

## Environment
- `NOTION_REFERENCE_DB_ID`: `34fc1cc7ec278178bf5ec4c5a7c1491f`
- `STORYBOARD_DB`: storyboard document/frame D1 binding
- `STORYBOARD_ASSETS_BUCKET`: storyboard image R2 binding
...
| table/bucket | role |
|---|---|
| `storyboard_documents` | title, related task id, version, memo, meta, export filename history |
| `storyboard_frames` | ordered page/frame content and image keys |
| `STORYBOARD_ASSETS_BUCKET` | thumbnail/image binaries under `storyboards/` |
...
## Creation
- Run `node scripts/create-reference-storyboard-dbs.cjs` only for the reference Notion DB.
